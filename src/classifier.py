...
class LDA:
    ...
    def fit(self, X, y):
        ...
        X_a = X[np.where(y == self.classes[0]), :].squeeze()
        X_b = X[np.where(y == self.classes[1]), :].squeeze()
        ...
        self._intercept = -(1 / 2) * (mean_A + mean_B).dot(inv_cov).dot(
            (mean_B - mean_A).T
        )
        self._weight = inv_cov.dot((mean_B - mean_A).T)

    def predict(self, X):
        """X : array-like of shape (n_samples, n_features)
        predicted data."""

        result = self._intercept + self._weight.dot(X.T)
        predict_classes = np.where(result < 0, self.classes[0], self.classes[1])
    ...
    def transform(self, X):
        return self._intercept + self._weight.dot(X.T)
